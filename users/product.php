...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_to_cart'])) {
    ...
    if ($product_pcs > 0) {
        $insert_sql = "INSERT INTO user_cart (username, product_pcs, product_id, product_name, product_price) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $insert_sql);
        mysqli_stmt_bind_param($stmt, "sisss", $username, $product_pcs, $product_id, $product_name, $product_price);

        if (mysqli_stmt_execute($stmt)) {
            // Update product quantity in the database
            $update_sql = "UPDATE products SET pcs = pcs - 1 WHERE id = ?";
            ...
            <?php if (mysqli_num_rows($result) > 0) : ?>
                <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                    <div class="bg-white rounded-lg shadow-md p-4">
                        ...
                        <p class="text-gray-700">Pcs: <?= $row['pcs'] ?></p>
                        ...
                        <form action="product.php?name=<?= $username ?>" method="post">
                            ...
                            <input type="hidden" name="product_pcs" value="<?= $row['pcs'] ?>">
